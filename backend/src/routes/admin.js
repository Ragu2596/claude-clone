...
import { getAllUserStats, getBusinessSummary } from '../services/costService.js';
...
async function isAdmin(req, res, next) {
  try {
    ...
    if (!admin || !admin.active) {
      res.setHeader('Access-Control-Allow-Origin', req.headers.origin || config.frontendUrl);
...
function otpVerified(req, res, next) {
  ...
  if (!sessionToken) { setCors(); return res.status(401).json({ error: 'OTP verification required' }); }
...
router.post('/send-otp', authenticate, isAdmin, async (req, res) => {
  try {
    ...
    otpStore.set(key, { otp, expiresAt, attempts: 0, role: req.adminRole, requestCount: (existing?.requestCount || 0) + 1, windowEnd: existing?.windowEnd || (Date.now() + 5 * 60 * 1000) });
    ...
    if (process.env.RESEND_API_KEY) {
      setImmediate(async () => {
        try {
          await sendEmail({
            to: email, subject: `🔐 rk.ai Admin OTP: ${otp}`,
            html: `<div style="font-family:system-ui,sans-serif;max-width:420px;margin:0 auto;padding:32px;background:#0f172a;border-radius:12px;"><h2 style="color:#f1f5f9;margin:0 0 6px">rk.ai Admin Login</h2><p style="color:#94a3b8;margin:0 0 6px">Role: <strong style="color:#c96442">${req.adminRole.toUpperCase()}</strong></p><div style="background:#1e293b;border-radius:8px;padding:24px;text-align:center;margin-bottom:20px;"><p style="color:#64748b;font-size:12px;margin:0 0 8px;text-transform:uppercase;letter-spacing:0.1em">Your OTP</p><p style="color:#c96442;font-size:48px;font-weight:900;letter-spacing:0.25em;margin:0">${otp}</p><p style="color:#64748b;font-size:12px;margin:8px 0 0">Expires in 10 minutes</p></div><p style="color:#475569;font-size:12px">If you did not request this, ignore this email.</p></div>`,
          });
...
router.post('/verify-otp', authenticate, isAdmin, async (req, res) => {
  try {
    ...
    otpStore.set(`session:${sessionToken}`, { email, role: entry.role, expiresAt: Date.now() + 2 * 60 * 60 * 1000 });
    otpStore.delete(key);
...
router.get('/users', authenticate, isAdmin, otpVerified, async (req, res) => {
  try {
    ...
    if (req.adminRole === 'viewer') {
      users = users.map(u => ({ ...u, email: u.email.replace(/(.{2}).*(@.*)/, '$1***$2') }));
    }
    res.json(users);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.get('/user/:id/logs', authenticate, isAdmin, otpVerified, async (req, res) => {
  try {
    ...
    const logs = await prisma.apiUsageLog.findMany({ where: { userId: req.params.id }, orderBy: { createdAt: 'desc' }, take: 50 });
    res.json(logs);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/admins', authenticate, isAdmin, otpVerified, isSuperAdmin, async (req, res) => {
  try {
    ...
    const admin = await prisma.adminUser.create({ data: { email, name: name || email.split('@')[0], role, addedBy: req.user.email, active: true } });
    console.log(`👤 New admin: ${email} (${role}) by ${req.user.email}`);
...
router.patch('/admins/:id', authenticate, isAdmin, otpVerified, isSuperAdmin, async (req, res) => {
  try {
    ...
    const updated = await prisma.adminUser.update({ where: { id: req.params.id }, data: { ...(role ? { role } : {}), ...(active !== undefined ? { active } : {}) } });
